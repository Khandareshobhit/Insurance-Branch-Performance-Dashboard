...
-- 14) Placed Achievement in Percentage
...
-- 15) Invoiced Achievement in Percentage
SELECT
	(CONCAT(
			 ROUND(
					((SELECT SUM(AMOUNT) FROM INVOICE)
					/
					(SELECT SUM(`New Budget`) + SUM(`Cross sell bugdet`)+ SUM(`Renewal Budget`) FROM individual_budget))*100
				,2)
	  ,"%")
	) AS "Invoiced_Achievement_%";
